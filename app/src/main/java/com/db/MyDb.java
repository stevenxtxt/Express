...
import com.db.sqlite.CursorUtils;
import com.db.sqlite.SqlBuilder;
import com.db.table.TableInfo;
...
public class MyDb {
	...
	private static Map<String,MyDb> daoMap = new HashMap<String, MyDb>();
	...
	private MyDb(DbConfig dbConfig){
		if(null == dbConfig){
	...
	public static MyDb create(DbConfig dbConfig){
		return getInstance(dbConfig);
	}
	
	public static MyDb create(Context ctx){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		return getInstance(dbConfig);
	}
	
	public static MyDb create(Context ctx,String dbName){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
	...
	public static MyDb create(Context ctx,String dbName,boolean isDebugSql){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		dbConfig.setDbName(dbName);
	...
	public static MyDb create(Context ctx,boolean isDebugSql){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		dbConfig.setDebugSql(isDebugSql);
		return getInstance(dbConfig);
	...
	private synchronized static MyDb getInstance(DbConfig dbConfig) {
		MyDb afeiDb = daoMap.get(dbConfig.getDbName());
		if(null == afeiDb){
			afeiDb = new MyDb(dbConfig);
	...
	private void debugSql(String sql) {
		if(dbConfig.isDebugSql()){ //dbConfig不可能为null
			Log.d("MyDb Debug Sql", ">>" + sql);
		}
	}


	public static class DbConfig{
		private String dbName="afei.db"; //数据库名称，默认为afei.db
