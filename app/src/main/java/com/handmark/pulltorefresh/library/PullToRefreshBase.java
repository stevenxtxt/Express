...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if(MotionEvent.ACTION_POINTER_DOWN==(event.getAction()& MotionEvent.ACTION_MASK)){
		    return false;
		}
	...
	public final void setPullLoadEnabled(boolean enable){
	    if(!enable&&mMode.getIntValue()>1){
	        setMode(Mode.mapIntToValue(mMode.getIntValue() - Mode.PULL_FROM_END.getIntValue()));
	    }else if(enable&& mMode.getIntValue()<2){
	        setMode(Mode.mapIntToValue(mMode.getIntValue() + Mode.PULL_FROM_END.getIntValue()));
	    }
	}
	@Override
